"""
从零训练一个分词器，分词器的作用主要是：
1. 对输入的序列进行分词，如“我爱你中国”被分词为“我”，“爱”，“你”，“中”，“国”
2. 将分词后的字符编码为次元，如将 “我”，“爱”，“你”，“中”，“国” 映射为 0 1 2 3 4
如何选择合适的分词器：
1. 直接用新华/牛津词典的优点是编码压缩率很好，缺点是页数太多
2. 自己训练的分词器，优点是词表长度和内容随意控制，缺点是压缩率很低
3. 为了避免模型词嵌入层参数占比太高，词表长度短短益善。在这里以实际情况为准
"""
...
def get_texts(data_path):
    """
    读取数据集
    由于数据集过大，一次性加载到内存中比较困难，使用 yield 一次读取一条数据
    :param data_path: json 格式数据集文件路径
    """
...
def train_tokenizer(data_path, tokenizer_dir, vocab_size):
    ...
    训练分词器
    :param data_path: json 格式数据集文件路径
    :param tokenizer_dir: 分词器权重的保存路径
    :param vocab_size: 词汇表的大小
    """
    tokenizer = Tokenizer(models.BPE())  # 使用 BPE 算法（字节对编码算法）
    # 这里对 BPE 算法进行解释
    # 核心思想是用更短的 ID 代表更频繁出现的片段，每次只合并频率最高的那一对
    # 直至词汇表长度达到规定值
...
if __name__ == "__main__":
    # train_tokenizer(
    #     data_path="dataset/pretrain_hq.jsonl",
    #     tokenizer_dir="weights/custom_tokenizer",
    #     vocab_size=VOCAB_SIZE,
    # )
    eval_tokenizer(tokenizer_dir="weights")
